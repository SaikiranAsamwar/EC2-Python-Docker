...
**Web Setup:** Access via SSH tunnel (`ssh -i key.pem -L 8080:localhost:8080 ec2-user@<EC2_IP>`), install suggested plugins, add credentials (dockerhub-credentials, kubeconfig, sonarqube-token), configure SonarQube server, create pipeline job pointing to Jenkinsfile in repository.
